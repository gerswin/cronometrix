...
const base = { name: 'dev', ip_address: '10.0.0.1', direction: 'entry' as const, last_seen_at: null, created_at: '', updated_at: '' }

describe('DeviceStatusSummary', () => {
  it('shows all online when no device is offline', () => {
    const devices: Device[] = [{ ...base, id: '1', status: 'online' as const }]
  ...
  it('shows red alert when all devices offline', () => {
    const devices: Device[] = [{ ...base, id: '1', status: 'offline' as const }]
    render(<DeviceStatusSummary devices={devices} />)
