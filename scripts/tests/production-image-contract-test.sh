...
grep -Fq 'COPY --from=selected-binary --chmod=0755 /app/target/release/cronometrix /usr/local/bin/cronometrix' "${API_DOCKERFILE}" || fail "API binary copy does not enforce executable mode"
