...
probe_function() {
  ...
  status="$(curl --silent --show-error \
    ...
    "${function_url}")"
  if [[ "${status}" != '400' ]]; then
    echo 'license Function invalid-body probe returned an unexpected status' >&2
    return 1
  fi
  if ! node -e '
    const { readFileSync } = require("node:fs");
    const body = JSON.parse(readFileSync(process.argv[1], "utf8"));
    if (body?.error?.code !== "BAD_REQUEST") process.exit(1);
  ' "${response_file}"; then
    echo 'license Function invalid-body probe returned an unexpected response' >&2
    return 1
  fi
}
