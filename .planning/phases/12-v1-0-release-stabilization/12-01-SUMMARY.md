...
- Tested implementation SHA: `be24f17b67916e8659a0a1a0daca5d5428dc3060`
...
4. Frontend install made reproducible with Node `24.15.0`, npm `11.12.1`,
   regenerated lockfile, Docker/web workflow alignment.
...
English copy (`Log in to Cronometrix`, `Username`, `Password`, `Log in`) while
the current UI renders Spanish copy (`Iniciar Sesion`, `Usuario`,
`Contrasena`). That remains a 12-02 functional-contract decision.
...
- Next step: merge this branch into the integration branch, then start 12-02
  from this tested SHA to fix the deterministic functional contracts.
